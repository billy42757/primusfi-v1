...
const ProgressBar: React.FC<ProgressBarProps> = ({ yesPercentage }) => {
  ...
  const yesBlocks = Math.round((yesPercentage / 100) * 50); // 30 blocks in total
  ...
  return (
    <div className="self-stretch flex-1 p-2.5 bg-[#111111] rounded-xl outline-1 outline-offset-[-1px] outline-[#313131] inline-flex justify-start items-center gap-1">
      {/* Yes Percentage */}
      <div className="w-8 justify-start text-[#3fd145] text-sm font-semibold font-interSemi leading-tight">
        {yesPercentage}%
      </div>
      ...
      <div className="flex-1 self-stretch flex justify-start items-center gap-[3px]">
        ...
        {Array.from({ length: yesBlocks }).map((_, index) => (
          <div
            key={`yes-${index}`}
            className="flex-1 self-stretch bg-[#3fd145] rounded-[100px]"
          />
        ))}
        ...
        {Array.from({ length: noBlocks }).map((_, index) => (
          <div
            key={`no-${index}`}
            className="flex-1 self-stretch bg-[#ff6464] rounded-[100px]"
          />
        ))}
      </div>

      {/* No Percentage */}
      <div className="w-8 text-right justify-start text-[#ff6464] text-sm font-semibold font-interSemi leading-tight">
        {noPercentage}%
      </div>
    </div>
  );
