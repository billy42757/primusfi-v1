...
const ProfileProposeItem: React.FC<ProfileProposeItemProps> = ({
  ...
  return (
    <div className="self-stretch p-6 bg-[#1e1e1e] rounded-2xl outline-1 outline-offset-[-1px] outline-[#313131] flex flex-col sm:flex-row justify-start items-start gap-6">
      <img className="w-12 h-12 rounded-lg" src={image} alt="proposal-icon" />
      <div className="flex-1 flex flex-col gap-2">
        <div className="flex justify-between items-center">
          <div className="text-[#07b3ff] text-base font-semibold font-interSemi leading-normal">{status}</div>
        </div>
        <div className="text-white text-lg font-medium font-satoshi leading-relaxed mb-2">{title}</div>
        <div className="flex flex-col gap-2">
          ...
          <div className="px-2 py-1 bg-[#838587]/20 rounded-xl flex items-center gap-2 w-fit">
            ...
            <div className="text-[#838587] text-xs font-medium font-satoshi leading-[18px]">{timeLeft}</div>
          </div>
        </div>
        <div className="flex flex-col gap-1 mt-2">
          ...
          <div className="w-[180px] sm:w-[220px] h-[18px] flex justify-between items-center mx-0 my-1">
            {Array.from({ length: 20 }).map((_, idx) => {
              const filledSegments = Math.floor((betAmount / totalAmount) * 20);
